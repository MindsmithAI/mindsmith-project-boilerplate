...
export default function FunnelList({ height }: { height: number }) {
  ...
  const segments = items.map((item, i) => {
    // Each segment is a trapezoid, base gets larger as i increases
    const y1 = svgHeight - (i * segmentHeight) + padding;
    const y2 = y1 - segmentHeight + spacing;
    const w1 = baseWidth * (i / (n));
    const w2 = baseWidth * ((i + 1) / (n)) - (spacing) * (1 - i / n);
